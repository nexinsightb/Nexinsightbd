"use server";

import { createClient } from "@/lib/supabase/server";
import { revalidatePath } from "next/cache";

export interface CreateRestaurantInput {
  name: string;
  slug: string;
  theme: string;
  manualStatus: "draft" | "active" | "inactive" | "disabled";
  mainTitle?: string;
  googleReviewUrl?: string;
  googleButtonText?: string;
  feedbackButtonText?: string;
  campaignEnabled: boolean;
  campaignTitle?: string;
  campaignDescription?: string;
  campaignButtonText?: string;
  facebookUrl?: string;
  instagramUrl?: string;
  whatsappUrl?: string;
  address?: string;
  phone?: string;
  openingTime?: string;
  closingTime?: string;
  closingText?: string;
  feedbackBackendUrl?: string;
  campaignBackendUrl?: string;
  plan?: string;
  startDate?: string;
  expiryDate?: string;
}

export interface ActionResult {
  success: boolean;
  error?: string;
  restaurantId?: string;
}

export async function createRestaurantAction(
  input: CreateRestaurantInput
): Promise<ActionResult> {
  try {
    const supabase = await createClient();

    // 1. Verify authenticated admin user
    const {
      data: { user },
    } = await supabase.auth.getUser();

    if (!user) {
      return { success: false, error: "Unauthorized access. Please log in again." };
    }

    const { data: profile } = await supabase
      .from("profiles")
      .select("role")
      .eq("id", user.id)
      .single();

    if (!profile || profile.role !== "admin") {
      return { success: false, error: "Forbidden. Admin privileges required." };
    }

    // 2. Validate & sanitize required fields
    const name = input.name?.trim();
    if (!name) {
      return { success: false, error: "Restaurant Name is required." };
    }

    const rawSlug = input.slug?.trim().toLowerCase();
    const sanitizedSlug = rawSlug ? rawSlug.replace(/[^a-z0-9-]/g, "") : "";
    if (!sanitizedSlug) {
      return { success: false, error: "A valid URL slug is required." };
    }

    // 3. Pre-check for duplicate slug
    const { data: existing, error: checkErr } = await supabase
      .from("restaurants")
      .select("id")
      .eq("slug", sanitizedSlug)
      .maybeSingle();

    if (checkErr) {
      console.error("[createRestaurantAction] Slug check error:", checkErr);
      return { success: false, error: "Database error while verifying URL availability." };
    }

    if (existing) {
      return {
        success: false,
        error: `The URL slug "${sanitizedSlug}" is already taken. Please choose a different restaurant URL.`,
      };
    }

    // 4. Insert into `restaurants` table
    const { data: restaurant, error: insertRestErr } = await supabase
      .from("restaurants")
      .insert({
        name,
        slug: sanitizedSlug,
        theme: input.theme || "theme-1",
        manual_status: input.manualStatus || "draft",
        brand_logo_path: null,
        square_logo_path: null,
        main_title: input.mainTitle?.trim() || null,
        google_review_url: input.googleReviewUrl?.trim() || null,
        google_button_text: input.googleButtonText?.trim() || null,
        feedback_button_text: input.feedbackButtonText?.trim() || null,
        campaign_enabled: Boolean(input.campaignEnabled),
        campaign_title: input.campaignTitle?.trim() || null,
        campaign_description: input.campaignDescription?.trim() || null,
        campaign_button_text: input.campaignButtonText?.trim() || null,
        facebook_url: input.facebookUrl?.trim() || null,
        instagram_url: input.instagramUrl?.trim() || null,
        whatsapp_url: input.whatsappUrl?.trim() || null,
        address: input.address?.trim() || null,
        phone: input.phone?.trim() || null,
        opening_time: input.openingTime?.trim() || null,
        closing_time: input.closingTime?.trim() || null,
        closing_text: input.closingText?.trim() || null,
      })
      .select("id")
      .single();

    if (insertRestErr || !restaurant) {
      console.error("[createRestaurantAction] Restaurant insert error:", insertRestErr);
      if (insertRestErr?.code === "23505") {
        return {
          success: false,
          error: `The URL slug "${sanitizedSlug}" is already registered.`,
        };
      }
      return {
        success: false,
        error: insertRestErr?.message || "Failed to create restaurant record.",
      };
    }

    const restaurantId = restaurant.id;

    // 5. Insert into `restaurant_backends` table
    const { error: backendErr } = await supabase
      .from("restaurant_backends")
      .insert({
        restaurant_id: restaurantId,
        feedback_backend_url: input.feedbackBackendUrl?.trim() || null,
        campaign_backend_url: input.campaignBackendUrl?.trim() || null,
      });

    if (backendErr) {
      console.error("[createRestaurantAction] Backends insert error:", backendErr);
      return {
        success: false,
        error: backendErr.message || "Failed to create restaurant backend record.",
      };
    }

    // 6. Insert into `subscriptions` table
    const startDateIso = input.startDate
      ? new Date(input.startDate).toISOString()
      : new Date().toISOString();
    const expiryDateIso = input.expiryDate
      ? new Date(input.expiryDate).toISOString()
      : null;

    const { error: subErr } = await supabase
      .from("subscriptions")
      .insert({
        restaurant_id: restaurantId,
        plan: input.plan || "free",
        start_date: startDateIso,
        expires_at: expiryDateIso,
        notes: "Created via Admin Dashboard",
      });

    if (subErr) {
      console.error("[createRestaurantAction] Subscriptions insert error:", subErr);
      return {
        success: false,
        error: subErr.message || "Failed to create subscription record.",
      };
    }

    revalidatePath("/admin");
    revalidatePath(`/r/${sanitizedSlug}`);

    return {
      success: true,
      restaurantId,
    };
  } catch (err: unknown) {
    console.error("[createRestaurantAction] Unexpected error:", err);
    const msg = err instanceof Error ? err.message : "An unexpected server error occurred.";
    return { success: false, error: msg };
  }
}
